...
import { marketingCampaignsAPI } from '../services/api';
...
export default function CampaignsPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const res = await marketingCampaignsAPI.list();
        setCampaigns(toList(res).map(unwrapPayload));
      } catch (err) {
        console.warn('Failed to load campaigns from API:', err.message);
      }
    };
    load();
  }, []);
  ...
  const handleSave = async () => {
    ...
    try {
      const created = await marketingCampaignsAPI.create({ localId: newItem.id, data: newItem });
      setCampaigns((prev) => [...prev, unwrapPayload(created)]);
    } catch (err) {
      console.warn('Failed to save campaign to API:', err.message);
